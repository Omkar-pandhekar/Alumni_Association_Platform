...
const Contact = () => {
  ...
  const HandleSubmit = (e) => {
    e.preventDefault();
    axios.post('http://localhost:3000/api/v1/feedback/contact',{
      fullname,
      email,
      feedback,
    })
    ...
    <Section id="contact">
      <div className="relative z-1 max-w-[50rem] mx-auto text-center mb-[3.875rem] md:mb-20 lg:mb-[6.25rem]">
        <div className="w-[2rem] max-lg:w-full h-full px-6 bg-n-8 border border-n-6 rounded-[2rem] lg:w-auto even:py-14 odd:py-8 odd:my-4 backdrop-blur-sm ">
          ...
          <form className="flex flex-col" action="/profile" onSubmit={HandleSubmit}>
            <input
            ...
            <Button className="hidden lg:flex"><button type="submit">Submit</button></Button>
          </form>
        </div>
      </div>
    </Section>
